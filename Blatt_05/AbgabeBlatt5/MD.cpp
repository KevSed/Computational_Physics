...
/*
Die Funktion Quadratgitter soll eine Matrix erstellen,
deren Spaltenanzahl der Teilchenanzahl entspricht,
...
Der Funktion wird ausserdem die Temperatur T uebergeben,
sodass die Geschwindigkeit einer Startenergie angepasst werden kann.
*/
...
MatrixXd initGeschwindigkeit(int Teilchenzahl, double T){
	...
	double Nf=2*Teilchenzahl-2; //Anzahl der Freiheitsgrade in 2d
	double T0=0;
	double alpha;
	double xs=0;
	double ys=0;
...
/*
Die Funktion LJForce gibt die Kraft aus,
welche nach dem Lennard-Jones-Potential auf ein Teilchen wirkt,
das relativ zu einem Anderen Teilchen um den Vektor r verschoben ist.
Ausserdem soll die Kraft null sein,
wenn die Teilchen um mehr als den kritischen radius von einander entfernt sind
 */
...
und die Bildteichen unetreinander ebenfalls mindestens um L von einander entfernt sind
...
MatrixXd Kraft(int Teilchenzahl, MatrixXd Position, double L, double rc){
	...
	// Dies funktioniert nur fuer rc<0.5L,
	//weil sonst auch eine Wechselwirkung mit mehr als einer Kopie des Teilchens vorliegen kann
...
/*
Die Funktion Paarkorrelation guckt fuer jedes Teilchen
ob es ein Bildteilchen von Teilchen 2 gibt,
welches einen geringeren Abstand zu Teilchen 1 hat als Teilchen 2 selbst.
Dann berechnet die Funktion den Abstand zu dem naechsten Teilchen, und ordnet den Abstand einem Bin zu.
Dies wird fuer jedes Teilchen mit allen Anderen Teilchen gemacht.
 */
...
/*
Die Funktion ---integrate-- soll einen Verlet-Algorithmus verwenden,
um die Bewegungen der Teilchen zu integrieren
Wir entscheiden uns fuer einen Geschwindigkeits-Verlet, da dieser selbststartend ist.
 */
...
void integrate(MatrixXd Position, MatrixXd Geschwindigkeit, double N, double h, int Teilchenzahl, string Energie, string Paarkorr, double L, bool Energiebetrachtung, int Nbins, int teq){
	...
	cout << "energiebetrachtung" << Energiebetrachtung << endl;
	...
	cout << "T_mean=  " << Tmean/N << endl;

	g=(1./counter)*g;
	rho=(Teilchenzahl)/(L*L);
...
/*
Aufgrund der Temperaturbetrachtung gehen wir davon aus,
dass die Aequilibrierungszeit bei h*N=5 abgeschlossen ist.

*/

	integrate(Teilchenvektor, TeilchengeschwT1, 1000000, h, Teilchenzahl, "EnergieT=0.txt", "gT=0.txt", L, true,3000,5);
	integrate(Teilchenvektor, TeilchengeschwT001, 1000000, h, Teilchenzahl, "EnergieT=0.txt", "gT=001.txt", L, false,3000,5);
	h=0.001;
	integrate(Teilchenvektor, TeilchengeschwT100, 1000000, h, Teilchenzahl, "EnergieT=0.txt", "gT=100.txt", L, false,3000,5);
